...
# Default output directory for export (empty = current working directory)
EXPORT_PATH="${EXPORT_PATH:-}"

# Directory to pick up release ZIPs for the 'release' command (optional)
...
cmd_export() {
    ...
    local output_dir="${2:-${EXPORT_PATH:-.}}"
...
# -----------------------------------------------------------------------------
# release — import a ZIP from RELEASE_PATH then publish it
#
# Combines import + publish in one step using the configured RELEASE_PATH.
# Looks for <project-name>.zip in RELEASE_PATH.
# -----------------------------------------------------------------------------
cmd_release() {
    local project_name="${1:-}"
    local project_desc="${2:-Imported by acumaticahelper.sh}"

    if [[ -z "$project_name" ]]; then
        echo "Usage: $0 release <project-name> [description]" >&2
        return 1
    fi

    if [[ -z "$RELEASE_PATH" ]]; then
        echo "Error: RELEASE_PATH is not configured in $_CONF_FILE" >&2
        return 1
    fi

    local zip_file="${RELEASE_PATH}/${project_name}.zip"
    if [[ ! -f "$zip_file" ]]; then
        echo "Error: release package not found: $zip_file" >&2
        return 1
    fi

    echo "Found release package: $zip_file"
    cmd_import "$zip_file" "$project_name" "$project_desc" || return 1
    cmd_publish "$project_name"
}
...
Commands:
  ...
  export  <project-name> [dir]       Export project to a ZIP file (default: EXPORT_PATH or .)
  import  <file.zip> [name] [desc]   Import a ZIP as a customization project
  release <project-name> [desc]      Import from RELEASE_PATH then publish
...
Optional tuning:
  ...
  RELEASE_PATH           Directory containing release ZIPs for the release command
